...
if __name__ == '__main__':
    ...
    result_np = np.genfromtxt(
        os.path.join(loading_dir, "results.csv"),
        delimiter=",",
        dtype=None,
        encoding=None,
        names=True
    ...
    metric_val_sorted_idx_np = result_np[f"val_{optimization_metric}"].argsort()

    # in case metric is an accuracy value, then invert sort order
    if optimization_metric_mode == 'max':
        metric_val_sorted_idx_np = metric_val_sorted_idx_np[::-1]

    if verbose:
        print(metric_val_sorted_idx_np)
    ...
    def create_load_ds(config):
        ...
        _, _, _ = fetch_loaders(config['data'], verbose=True, use_multiprocess=False)
    ...
    searcher = FinalSearcher(
        {
            f"rank{i+1}_{result_np['logdir'][idx]}": import_as(
                os.path.join(os.path.abspath(test_config['loading_dir']), f"{result_np['logdir'][idx]}.yaml"),
                loading_type='yaml'
            )
            for i, idx in enumerate(metric_val_sorted_idx_np[:n_best])
        }
    )
