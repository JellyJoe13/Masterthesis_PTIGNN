...
def getNodeFeatures(list_rdkit_atoms, owningMol):
    ...
    global_tags = dict(
        rdkit.Chem.FindMolChiralCenters(owningMol, force=True, includeUnassigned=True, useLegacyImplementation=False))
    ...
    for node_index, node in enumerate(list_rdkit_atoms):
        ...
        features += one_hot_embedding(global_chiral_tag,
                                      [0, 1, 2])  # chiral tag of atom, dim=3+1 (global chiral features)
